...
/** Manual diary entries can be edited/deleted; task-generated events are read-only. */
export function canEditEvent(event: Pick<GardenEvent, 'taskId'>): boolean {
  return event.taskId === null;
}
...
export function formatEventHeadline(event: Pick<GardenEvent, 'type' | 'taskId'>): string {
  ...
  const prefix = isTaskGeneratedEvent(event) ? '✓ ' : emoji ? `${emoji} ` : '';
  return `${prefix}${label}`;
}
